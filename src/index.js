...
// Save native console.table BEFORE any global override — prevents infinite loop
const _nativeTable = console.table.bind(console);
...
function dispatch(level, args, meta) {
    ...
    if (process.env.NODE_ENV === "production") {
        const entry = {
            ...
            caller: getCallerInfo().replace(/[()]/g, ""),
...
const smartConsole = {
    ...
    child(meta = {}) {
        ...
        return {
            log(...args) {
                dispatch("log", args, boundMeta);
            },
            info(...args) {
                dispatch("info", args, boundMeta);
            },
            warn(...args) {
                dispatch("warn", args, boundMeta);
            },
            error(...args) {
                dispatch("error", args, boundMeta);
            },
            debug(...args) {
                dispatch("debug", args, boundMeta);
            },
            success(...args) {
                dispatch("success", args, boundMeta);
            },
